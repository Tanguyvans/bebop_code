...
#récupération de l'image et conversion dans un format utilisable avec openCV
def image_callback(ros_image):
  ...
  getContours2(imgCanny, cv_image)
...
#fonction qui permet de connaitre l'orientation d'une flèche et d'envoyer des commandes au drone en fonction de cela
def getContours(imgCanny, img):
    ...
    for cnt in contours:
        ...
        if area>500:
            ...
            objectType = "None"
#même chose que la fonction d'avant avec qq améliorations
def getContours2(imgCanny, img):
    #on applique la fonction findCountours d'openCv pour obtenir tous les contours présents sur l'image qu'on analyse
    _,contours, hierarchy = cv2.findContours(imgCanny,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE) 
...
def main(args):
  ...
  rospy.init_node('image_bebop', anonymous=True)
